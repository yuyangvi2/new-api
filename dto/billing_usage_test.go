...
func TestProtocolUsageDoesNotSerializeBillingUsage(t *testing.T) {
	openAIUsage := Usage{PromptTokens: 1, BillingUsage: NewClaudeMessagesBillingUsage(&ClaudeUsage{InputTokens: 9})}
	openAIData, err := common.Marshal(openAIUsage)
	require.NoError(t, err)
	assert.NotContains(t, string(openAIData), "billing_usage")

	claudeUsage := ClaudeUsage{InputTokens: 1, BillingUsage: NewOpenAIChatBillingUsage(&Usage{PromptTokens: 8})}
	claudeData, err := common.Marshal(claudeUsage)
	require.NoError(t, err)
	assert.NotContains(t, string(claudeData), "billing_usage")

	geminiUsage := GeminiUsageMetadata{PromptTokenCount: 1, BillingUsage: NewOpenAIChatBillingUsage(&Usage{PromptTokens: 7})}
	geminiData, err := common.Marshal(geminiUsage)
	require.NoError(t, err)
	assert.NotContains(t, string(geminiData), "billing_usage")
}
