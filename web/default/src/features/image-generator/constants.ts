...
export function isVipeakSeedanceVideoModel(model: string): boolean {
  const normalizedModel = model.toLowerCase().trim()
  return (
    normalizedModel === 'dreamina-seedance-2-0-260128' ||
    normalizedModel === 'dreamina-seedance-2-0-fast-260128' ||
    normalizedModel === 'seedance' ||
    normalizedModel === 'seedance-fast'
  )
}

export function isToAPIsSeedanceVideoModel(model: string): boolean {
  const normalizedModel = model.toLowerCase().trim()
  return (
...
export function isOfficialSeedanceVideoModel(model: string): boolean {
  const normalizedModel = model.toLowerCase().trim()
  return (
    normalizedModel === 'doubao-seedance-2-5' ||
...
export function getSeedanceResolutionOptions(
  ...
  const isFastOrMini =
    normalizedModel === 'seedance-2-fast' ||
    normalizedModel === 'seedance-2-mini' ||
    normalizedModel === 'seedance_2.0_fast' ||
    ...
    normalizedModel === 'dreamina-seedance-2-0-fast-260128' ||
    normalizedModel === 'seedance-fast' ||
    normalizedModel === 'seedance2.0_fast_direct' ||
    normalizedModel === 'seedance2.0_fast_vision' ||
    normalizedModel === 'seedance2.0_fast_mini'
  if (isFastOrMini) {
    return options.filter((option) =>
      ['480p', '720p'].includes(option.value.toLowerCase())
    )
  }
  return options
}
...
export function getSeedancePricePerMillionCNY({
  ...
  if (normalizedModelKey === 'doubao-seedance-2-5') {
    if (normalizedResolution === '1080p') return hasVideoInput ? 46 : 77
    if (normalizedResolution === '480p' || normalizedResolution === '720p') {
      return hasVideoInput ? 42 : 70
    }
    return undefined
  }
