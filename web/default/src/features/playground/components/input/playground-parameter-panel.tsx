...
export function PlaygroundParameterPanel(props: PlaygroundParameterPanelProps) {
  ...
  // Use a single Base UI Trigger with `render={button}` (same pattern as
  // flow-node-filter / PromptInputActionMenuTrigger). Nesting
  // TooltipTrigger(render=PopoverTrigger(render=button)) can drop the visible
  // button under Base UI's composition, so keep the tooltip as native title.
  const triggerButton = (
    <PromptInputButton
      aria-label={t('Parameters')}
      className='text-muted-foreground hover:text-foreground hover:bg-muted/70 relative gap-1.5 font-medium'
