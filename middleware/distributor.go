			...
			if shouldSelectChannel {
				...
				if channel == nil {
					...
					if err != nil {
						...
						logger.LogError(c, fmt.Sprintf("failed to select channel: group=%s model=%s error=%s", showGroup, modelRequest.Model, err.Error()))
						abortWithOpenAiMessage(c, http.StatusServiceUnavailable, i18n.T(c, i18n.MsgDistributorRouteFailed), types.ErrorCodeGetChannelFailed)
						return
					}
					if channel == nil {
						logger.LogError(c, fmt.Sprintf("no available channel: group=%s model=%s", usingGroup, modelRequest.Model))
...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	} else if strings.HasPrefix(c.Request.URL.Path, "/v1/seedance-m/") {
