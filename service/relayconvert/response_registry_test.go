...
func TestConvertStreamResponseStatefulMultiHopResponsesToClaude(t *testing.T) {
	...
	require.NotEmpty(t, finalResults)
	var sawStopBlock, sawMessageDelta, sawMessageStop bool
	for _, result := range finalResults {
		claudeResponse, ok := result.Value.(*dto.ClaudeResponse)
		if !ok || claudeResponse == nil {
			continue
		}
		sawStopBlock = sawStopBlock || claudeResponse.Type == "content_block_stop"
		sawMessageDelta = sawMessageDelta || claudeResponse.Type == "message_delta"
		sawMessageStop = sawMessageStop || claudeResponse.Type == "message_stop"
	}
	assert.True(t, sawStopBlock)
