...
func TestConvertRequestResponsesToGeminiAppliesResponsesPreprocess(t *testing.T) {
	...
	req := &dto.OpenAIResponsesRequest{
		Model: "gemini-test",
		Input: mustRegistryRawMessage(t, []map[string]any{
			{
				"role":    "user",
				"content": "next turn",
			},
		...
		Tools: mustRegistryRawMessage(t, []map[string]any{
...
func TestConvertRequestResponsesToGeminiUsesDirectConverter(t *testing.T) {
	...
	req := &dto.OpenAIResponsesRequest{
		Model:           "gemini-test",
		Instructions:    mustRegistryRawMessage(t, "system rules"),
		...
		Text: mustRegistryRawMessage(t, map[string]any{
...
func TestConvertRequestResponsesToClaudeUsesDirectConverter(t *testing.T) {
	...
	req := &dto.OpenAIResponsesRequest{
		...
		Stream:            &stream,
		MaxOutputTokens:   &maxOutputTokens,
		ParallelToolCalls: mustRegistryRawMessage(t, parallelToolCalls),
...
func mustRegistryRawMessage(t *testing.T, value any) []byte {
	t.Helper()
