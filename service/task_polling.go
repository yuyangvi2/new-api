...
type TaskBillingAdjustmentWithClamp interface {
...
func SettleTaskBillingOnComplete(ctx context.Context, adaptor TaskPollingAdaptor, task *model.Task, taskResult *relaycommon.TaskInfo) {
	...
	if adjustable, ok := adaptor.(TaskBillingAdjustmentWithClamp); ok {
		if actualQuota, clamp := adjustable.AdjustBillingOnCompleteWithClamp(task, taskResult); actualQuota > 0 {
