...
func attestTopTca(token string, config ClientConfig) (string, error) {
	...
	raResults, err := requestTop(&topInfo, "GetAttestationBackend", headers, jsonBody, config.HTTPClient)
	if err != nil {
...
func validateRaInfo(raInfo map[string]any) bool {
	...
	if evidence, ok := raInfo["evidence"].(string); ok {
		...
		if err := common.Unmarshal([]byte(evidence), &evidenceJSON); err != nil {
	...
	if err := common.Unmarshal(tokenBody, &tokenClaims); err != nil {
...
func parseResponse(body io.Reader) (map[string]interface{}, error) {
	var data map[string]interface{}
	if err := common.DecodeJson(body, &data); err != nil {
